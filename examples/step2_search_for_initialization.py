...
def run_cpp_matcher(img1_path, img2_path):
    """Run C++ search_init_bin, return match count."""
    result = subprocess.run([BINARY, img1_path, img2_path],
...
def main():
    ...
    cpp_n = run_cpp_matcher(IMAGE1, IMAGE2)
    kps1_cpp, descs1_cpp = run_cpp_extractor(IMAGE1)
    kps2_cpp, descs2_cpp = run_cpp_extractor(IMAGE2)
    ...
    print(f"  Matches          : {cpp_n}")

    # ── Python (own keypoints) ────────────────────────────────────────────
    sep("Python (own keypoints)")
    img1, kps1_py, descs1_py = run_python_extractor(IMAGE1)
    img2, kps2_py, descs2_py = run_python_extractor(IMAGE2)
    ...
    # ── Python matching logic with C++ keypoints ──────────────────────────
    ...
    print(f"  Matches          : {n_iso}  "
          f"({'== C++' if n_iso == cpp_n else f'diff {n_iso - cpp_n:+d} vs C++'})")

    # ── Summary ───────────────────────────────────────────────────────────
    sep("Summary")
    print(f"  {'':30s}  {'matches':>7}  {'vs C++':>8}")
    sep()
    print(f"  {'C++ (reference)':30s}  {cpp_n:>7}")
    print(f"  {'Python (own kps)':30s}  {n_py:>7}  {n_py-cpp_n:>+8d}")
    print(f"  {'Python matcher + C++ kps':30s}  {n_iso:>7}  {n_iso-cpp_n:>+8d}")
