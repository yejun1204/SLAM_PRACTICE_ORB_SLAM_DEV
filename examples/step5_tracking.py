"""
Step 5: TrackWithMotionModel + TrackLocalMap

Pipeline:
  1. Initialization (same as step4)
  2. Per frame:
       a. TrackWithMotionModel  — project last frame's map points, th=15
       b. TrackLocalMap         — project remaining map points, th=1, re-optimize
       success: n_inliers >= 30 (monocular without IMU)
  3. last_frame_mps updated from TrackLocalMap inliers each frame
"""
...
def build_map(init_result):
    ...
    for i, (p3d, is_tri) in enumerate(zip(points3d, tri)):
        ...
        if j < 0:
            continue
        map_points.append(MapPoint(pos=p3d, descriptor=descs_cur[j],
                                   octave=kps_cur[j].octave))
    return T_cw_cur, map_points
...
def main():
    ...
    T_cw_last, map_points = build_map(init)
    print(f"  Map: {len(map_points)} points")

    velocity       = np.eye(4, dtype=np.float64)
    # First frame uses all map points (same as initialization cur frame's observations)
    last_frame_mps = list(map_points)

    sep("Phase 2 — TrackWithMotionModel + TrackLocalMap")
    print(f"  {'frame':>5}  {'MM':>6}  {'LM':>6}  {'new':>5}  C")
    sep()
    ...
    for cur_idx in range(start_idx, min(len(FRAMES), MAX_FRAMES)):
        ...
        if T_cw is None:
            print(f"  [{cur_idx:4d}]  MM FAIL  inliers={n_mm}")
            n_fail += 1
            if n_fail >= 3:
                print("  Tracking LOST."); break
            continue

        # Stage 2: TrackLocalMap (all map points, th=1)
        ...
        # MM 인라이어 중 LM PoseOpt에서 탈락한 것은 여기서 제외된다.
        ...
        print(f"  [{cur_idx:4d}]  MM={n_mm:3d}   LM={n_lm:3d}  +{n_new:3d}  "
              f"C=[{C[0]:6.3f} {C[1]:6.3f} {C[2]:6.3f}]")

        vis = draw_reprojection(img_cur, map_points, T_cw, lm_matches)
        cv2.putText(vis, f"frame {cur_idx}  MM={n_mm} LM={n_lm} | any key: next  ESC: quit",
